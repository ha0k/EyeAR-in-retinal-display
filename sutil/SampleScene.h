...
public:
	  float xQuadPos;//-0.1,-0.07
  float yQuadPos;//-0.24; -0.16,0.13
  double xWindowOffset;
  double xAdjustWindowOffset;
  double yWindowOffset;
  double yAdjustWindowOffset;
  bool isNewMeasurement;
  bool isNewFOV;
  float3 upVec;
  double glutFocus;
  double glutAperture;
  double curSceneFocusDepth;
  double curSceneApertureSize;
  int RayFrameTotal;
  int sampleSize;
  int textureWidth;
  int textureHeight;
  int textureDepth;
  bool useEnhancedAperture;
  float ratioFOV;
  ...
  struct InitialCameraData
  {
	...
	SUTILAPI InitialCameraData( float3 m_eye, float3 m_lookat, float3 m_up, float  m_vfov, float m_hfov)
  ...
  // VCA
  SUTILAPI SampleScene( const VCAOptions& vca_options );

  SUTILAPI virtual ~SampleScene() {sampleSize=8;}
  ...
  SUTILAPI void  setUsePBOBuffer( bool onoff ) { m_use_pbo_buffer = onoff; }
  SUTILAPI bool  usesPBOBuffer() { return m_use_pbo_buffer; }

  SUTILAPI void  setAccelDescriptor(const AccelDescriptor& desc) { m_accel_desc = desc; }
    SUTILAPI virtual void SetCameraData();
  ...
  //----------------------------------------------------------------------------
	SUTILAPI virtual void updateROS();
	SUTILAPI virtual void updateTexture();
	SUTILAPI virtual void getFramePixels();
  SUTILAPI virtual void  renderCross();
  SUTILAPI virtual void publishResult();
  ...
  SUTILAPI virtual unsigned int getImageWidth() const { return mImageWidth; }
  SUTILAPI virtual unsigned int getImageHeight() const { return mImageHeight; }

  // Optional methods for VCA rendering.
  SUTILAPI virtual optix::Buffer getProgressiveStreamBuffer() { return 0; }
...
protected:
  ...
  // VCA mode, must be set in constructor
  const bool   m_vca_enabled;
  optix::RemoteDevice m_vca_remote_device;
...
private:
  ...
  //  Defaults
  SUTILAPI const static int IMAGE_WIDTH;
  SUTILAPI const static int IMAGE_HEIGHT;
};
